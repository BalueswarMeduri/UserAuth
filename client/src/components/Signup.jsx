...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error("Email is already there ,Registration failed. Please try again.");
        return;
      }

      toast.success("Registration successful! Redirecting to login...");
      setTimeout(() => navigate('/login'), 2000); // Wait for 2 seconds
    } catch (error) {
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <button
              type="submit"
              className="flex w-full justify-center rounded-md bg-[#fac638] px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-yellow-500 transition"
            >
              Sign up
            </button>
          </div>
